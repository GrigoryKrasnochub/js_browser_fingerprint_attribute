...
function getOpenPorts(portsToScan = Ports) {
  let scannedPortsResult = {};
  let doPortsScan = function (resolve) {
    let startIndex = Object.keys(scannedPortsResult).length;
    let promises = [];
    for (let i = startIndex; i < startIndex + PortsPerIteration; i++) {
      if (i >= portsToScan.length) {
        break;
      }
      promises.push(portIsOpen('127.0.0.1', portsToScan[i]));
    }
    if (promises.length > 0) {
      Promise.all(promises).then(values => {
        ...
        if (Object.keys(scannedPortsResult).length < portsToScan.length) {
          doPortsScan(resolve);
        } else {
